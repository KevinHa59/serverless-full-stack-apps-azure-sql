...
async function GetMonitoredRoutes() {
  ...
  const queryResult = await pool
    .request()
    .execute('web.GetMonitoredRoutes');
  var monitoredRutes = JSON.parse(
    queryResult.recordset[0]['MonitoredRoutes']
  );
...
async function ProcessGeoFences(context, buses) {
  ...
  var geoFences = JSON.parse(
    queryResult.recordset[0]['ActivatedGeoFences']
  );
  if (geoFences == null) return [];

  context.log(
    `Found ${geoFences.length} buses activating a geofence`
  );
